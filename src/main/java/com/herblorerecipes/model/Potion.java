...
public enum Potion
{
	...
	SUPERANTIPOISON("Irit leaf", "Unicorn horn dust", "Irit potion (unf)", "Superantipoison", 48),
	...
	static
	{
		...
		Arrays.stream(values()).forEach(potion -> potionNameToPotionMap.put(KEY_POTION_IDENTIFIER + potion.potionName, potion));
		...
		Arrays.stream(values()).forEach(potion -> potionNames.add(potion.potionName));
